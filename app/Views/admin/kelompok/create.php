...
<div class="container-fluid">
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        <h1 class="h3 mb-0 text-gray-800">Tambah Kelompok Supervisi</h1>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3"><h6 class="m-0 font-weight-bold text-primary">Form Kelompok</h6></div>
        <div class="card-body">
            <form action="<?= base_url('admin/kelompok/store'); ?>" method="post">
                <?= csrf_field(); ?>
                <div class="row">
                    <div class="col-md-6">
                        <div class="form-group">
                            <label for="nama_kelompok">Nama Kelompok</label>
                            <input type="text" class="form-control" id="nama_kelompok" name="nama_kelompok" value="<?= esc(old('nama_kelompok')); ?>" placeholder="Contoh: Kelompok 1 - Matematika" required maxlength="100">
                        </div>
                        <div class="form-group">
                            ...
                            <?php if (empty($supervisors)): ?>
                                <small class="text-danger">Belum ada user supervisor/kepala yang Aktif.</small>
                            <?php endif; ?>
                        </div>
                        <div class="form-group">
                            ...
                            <select class="form-control" id="tahun_ajar_id" name="tahun_ajar_id">
                                ...
                                <?php foreach ($tahunAjars as $t): ?>
                                    <option value="<?= $t['id']; ?>" <?= (old('tahun_ajar_id') !== null ? old('tahun_ajar_id') == $t['id'] : (isset($tahunAktif) && $tahunAktif && $tahunAktif['id'] == $t['id'])) ? 'selected' : ''; ?>>
                                        <?= esc($t['tahun_ajar']); ?> - <?= esc($t['semester']); ?><?= (isset($tahunAktif) && $tahunAktif && $tahunAktif['id'] == $t['id']) ? ' (Aktif)' : ''; ?>
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                                ...
                                <?php if (!empty($jenisPenilaians)): ?>
                                    <?php foreach ($jenisPenilaians as $jp): ?>
                                        <div class="custom-control custom-checkbox mb-2">
                                            <input type="checkbox" class="custom-control-input" id="jp_<?= $jp['id'] ?>" name="jenis_penilaian_ids[]" value="<?= $jp['id'] ?>" <?= in_array($jp['id'], (array)old('jenis_penilaian_ids', [])) ? 'checked' : '' ?>>
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control" id="anggota_ids" name="anggota_ids[]" multiple size="12">
                                <?php foreach ($gurus as $g): ?>
                                    <option value="<?= $g['id']; ?>"><?= esc($g['nama']); ?><?= !empty($g['mata_pelajaran']) ? ' - ' . esc($g['mata_pelajaran']) : ''; ?></option>
                                <?php endforeach; ?>
                            </select>
                            <?php if (empty($gurus)): ?>
                                <small class="text-danger">Data guru kosong.</small>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
                <div class="form-group text-right">
                    <button type="submit" class="btn btn-primary"><i class="fas fa-save"></i> Simpan Kelompok</button>
                </div>
            </form>
        </div>
    </div>
