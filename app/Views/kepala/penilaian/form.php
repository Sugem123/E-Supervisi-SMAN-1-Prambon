...
<div class="container-fluid">
    ...
    <div class="d-sm-flex align-items-center justify-content-between mb-4">
        ...
        <div>
            <a href="<?= base_url('kepala/foto-bukti/upload/' . (isset($schedule['id']) ? $schedule['id'] : '')) ?>" class="btn btn-info btn-sm shadow-sm mr-2">
                <i class="fas fa-camera mr-1"></i> Upload Foto Bukti
            </a>
            <a href="<?= base_url('kepala/jadwal') ?>" class="btn btn-secondary btn-sm shadow-sm">
                <i class="fas fa-arrow-left fa-sm mr-1"></i> Kembali ke Jadwal
            </a>
        </div>
    </div>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 bg-white">
            <h6 class="m-0 font-weight-bold text-primary">
                <i class="fas fa-chalkboard-teacher mr-1"></i> Informasi Guru
            </h6>
        </div>
        <div class="card-body">
            <div class="row">
                ...
                <div class="col-md-6">
                    <table class="table table-borderless mb-0">
                        <tr>
                            <td width="35%"><strong>Tanggal Supervisi</strong></td>
                            <td width="5%">:</td>
    ...
    <div class="card shadow mb-4">
        <div class="card-header py-3 bg-white">
            <h6 class="m-0 font-weight-bold text-primary">
                <i class="fas fa-tasks mr-1"></i> Penilaian Supervisi (4 Tahap)
            </h6>
        </div>
        <div class="card-body">
            <!-- Nav tabs -->
            ...
            <div class="tab-content pt-4" id="assessmentTabsContent">
                    ...
                    <?php foreach ($jenisPenilaian as $index => $jenis): ?>
                        ...
                        <div class="tab-pane fade <?= $index == 0 ? 'show active' : '' ?>" 
                            ...
                            <div class="alert alert-info shadow-sm mb-3">
                                <strong><i class="fas fa-magic mr-1"></i> Fitur Edit Massal (Bulk Edit)</strong>
                                <div class="form-row align-items-center mt-2">
                                    <div class="col-md-4">
                                        <label for="bulk-skor-<?= $jId ?>" class="small font-weight-bold">Set Semua Skor Komponen Ini ke:</label>
                                    ...
                                    <div class="col-md-4">
                                        ...
                                        <button type="button" class="btn btn-primary btn-sm bulk-edit-apply-btn" data-tab-id="<?= $jId ?>">
                                            <i class="fas fa-check mr-1"></i> Terapkan ke Semua
                                        </button>
                                    </div>
                                </div>
                            </div>
                            ...
                            <form id="form-<?= $jId ?>" class="form-step-penilaian">
                                ...
                                <input type="hidden" name="jenis_penilaian_id" value="<?= $jId ?>">
                                ...
                                <div class="table-responsive">
                                    <table class="table table-bordered table-hover" style="table-layout: auto; width: 100%;">
                                        ...
                                        <tbody>
                                            <?php $no = 1; ?>
                                            <?php if (isset($aspekByJenis[$jId]) && is_array($aspekByJenis[$jId])): ?>
                                                <?php foreach ($aspekByJenis[$jId] as $aspek): ?>
                                                    <?php 
                                                        $aspekId = isset($aspek['id']) ? $aspek['id'] : '';
                                                        $existingDetail = isset($existingDetails[$jId][$aspekId]) ? $existingDetails[$jId][$aspekId] : null;
                                                    ?>
                                                    <tr>
                                                        <td class="text-center align-middle"><?= $no++ ?></td>
                                                        <td class="align-middle" style="white-space: normal; word-wrap: break-word;">
                                                            <strong><?= isset($aspek['nama_aspek']) ? esc($aspek['nama_aspek']) : '' ?></strong>
                                                        </td>
                                                        <td class="align-middle">
                                                            <select name="skor_<?= $aspekId ?>" class="form-control skala-dropdown" data-aspek="<?= $aspekId ?>" required>
                                                        ...
                                                        <td class="align-middle">
                                                            <textarea name="catatan_<?= $aspekId ?>" class="form-control catatan-field" data-aspek="<?= $aspekId ?>" rows="1" placeholder="Catatan akan terisi otomatis"><?= $existingDetail && isset($existingDetail['catatan']) ? esc($existingDetail['catatan']) : '' ?></textarea>
                                                        </td>
                                                    </tr>
                                                <?php endforeach; ?>
                                            <?php endif; ?>
                                ...
                                <div class="form-group mt-3">
                                    <label for="rekomendasi-<?= $jId ?>"><strong><i class="fas fa-comment-dots mr-1"></i> Rekomendasi Perbaikan:</strong></label>
                                    <textarea name="rekomendasi" id="rekomendasi-<?= $jId ?>" class="form-control" rows="3" placeholder="Tuliskan rekomendasi perbaikan untuk komponen ini..."><?= isset($existingResults[$jId]['rekomendasi']) ? esc($existingResults[$jId]['rekomendasi']) : '' ?></textarea>
                                </div>
                ...
                <div class="tab-pane fade <?= (isset($activeTab) && $activeTab == 'foto') ? 'show active' : '' ?>" 
                    ...
                    <div class="d-flex justify-content-between align-items-center mb-3">
                        ...
                        <a href="<?= base_url('kepala/foto-bukti/upload/' . (isset($schedule['id']) ? $schedule['id'] : '')) ?>" class="btn btn-outline-primary btn-sm">
    ...
    <div class="card shadow mb-4 border-left-success">
        <div class="card-header py-3 bg-white">
            <h6 class="m-0 font-weight-bold text-success">
                <i class="fas fa-check-circle mr-1"></i> Status & Penyelesaian Supervisi
            </h6>
        </div>
        <div class="card-body">
            <div class="row align-items-center">
                <div class="col-md-8">
                    <p class="mb-1 text-gray-800">
                        Setelah menyelesaikan semua tahap penilaian, silakan klik tombol di samping untuk menyelesaikan proses supervisi.
                    </p>
                    <small class="text-muted">
                        <i class="fas fa-info-circle mr-1"></i> Sistem akan menghitung akumulasi total skor, menetapkan predikat kualifikasi, dan mengupdate status jadwal menjadi <strong>Selesai</strong>.
                    </small>
                </div>
                <div class="col-md-4 text-md-right mt-3 mt-md-0">
                    <?php if (isset($schedule['status']) && $schedule['status'] == 'Terjadwal'): ?>
                        <button id="completeBtn" class="btn btn-success btn-lg shadow-sm">
                            <i class="fas fa-check-double mr-1"></i> Selesaikan Supervisi
                        </button>
                    <?php else: ?>
                        <button class="btn btn-success btn-lg shadow-sm" disabled>
                            <i class="fas fa-check-double mr-1"></i> Supervisi Sudah Selesai
                        </button>
                    <?php endif; ?>
                </div>
            </div>
...
$(document).ready(function() {
    ...
    $('#completeBtn').on('click', function() {
        Swal.fire({
            ...
            text: "Apakah Anda yakin ingin menyelesaikan supervisi ini? Setelah diselesaikan, data penilaian tidak dapat diubah.",
            icon: 'warning',
            ...
            if (result.isConfirmed) {
                ...
                var originalBtnHtml = completeBtn.html();
                completeBtn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin mr-1"></i> Memproses...');
                
                $.ajax({
                    url: '<?= base_url('kepala/penilaian/complete/' . (isset($schedule['id']) ? $schedule['id'] : '')) ?>',
                    method: 'POST',
                    data: {
                        [csrfName]: csrfHash
                    },
                    ...
                    success: function(response) {
                        if (response.status === 'success') {
                            Swal.fire({
                                icon: 'success',
                                title: 'Berhasil!',
                                text: response.message || 'Supervisi berhasil diselesaikan!',
                                timer: 2000,
                                showConfirmButton: false
                            }).then(() => {
                                window.location.href = response.redirect || '<?= base_url('kepala/jadwal') ?>';
                            });
                        } else {
                            Swal.fire({
                                icon: 'error',
                                title: 'Gagal',
                                text: 'Gagal menyelesaikan supervisi: ' + (response.message || 'Unknown error')
                            });
                            if (response.csrf_token) {
                                updateCsrfToken(response.csrf_token);
                            } else if (response.token) {
                            ...
                            completeBtn.prop('disabled', false).html(originalBtnHtml);
                        }
                    },
                    error: function(xhr, status, error) {
    ...
    $('#quickPhotoUploadForm').on('submit', function(e) {
        ...
        $.ajax({
            url: '<?= base_url('kepala/foto-bukti/upload/' . (isset($schedule['id']) ? $schedule['id'] : '')) ?>',
            type: 'POST',
            data: formData,
            processData: false,
            contentType: false,
    ...
    $(document).on('click', '.btn-delete-quick-photo', function() {
        ...
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: '<?= base_url('kepala/foto-bukti/delete/') ?>/' + photoId,
                    type: 'POST',
                    data: {
                        [csrfName]: csrfHash
                    },
                    ...
                    success: function(res) {
                        ...
                        if (res.status === 'success') {
                            ...
                            $('.photo-card-' + photoId).fadeOut(400, function() {
                                $(this).remove();
                                var remaining = $('#quickPhotoGallery .photo-card-' + photoId).length;
